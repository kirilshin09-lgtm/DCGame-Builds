...
// Begin JS library code
  ...
  function _MentorOpenUrlNewTab(urlPtr) {
      var url = UTF8ToString(urlPtr);
      var win = window.open(url, "_blank");
      if (!win) {
        // попап всё же заблокирован — уводим текущую вкладку (хуже, но работает)
        window.location.href = url;
      }
    }

  function _WebPromptText(titlePtr, defPtr) {
      var res = window.prompt(UTF8ToString(titlePtr), UTF8ToString(defPtr));
      if (res === null) res = UTF8ToString(defPtr);   // «Отмена» — оставляем как было
...
var wasmImports = {
  ...
  /** @export */
  MentorOpenUrlNewTab: _MentorOpenUrlNewTab,
  /** @export */
  WebPromptText: _WebPromptText,
  /** @export */
